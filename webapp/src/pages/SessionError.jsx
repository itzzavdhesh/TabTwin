...
export default function SessionError({ type = 'not-found' }) {
  ...
  let message =
    'The session link is invalid, has expired, or the host has ended the session.';

  if (type === 'network') {
    title = 'Connection Failed';
    message =
      "We couldn't connect to the session. Please check your network and try again.";
  } else if (type === 'ended') {
    title = 'Session Ended';
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-slate-50 px-6 py-10 text-center">
      ...
      <p className="mt-3 max-w-sm text-base text-slate-600 leading-relaxed">
        {message}
      </p>
