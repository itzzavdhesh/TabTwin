...
export default function Session({ sessionId }) {
  ...
  const {
    ...
    enabled: Boolean(
      session.onboarding?.enabled && session.onboarding?.guidance,
    ),
      ...
      <section className="mx-auto flex min-h-screen max-w-6xl items-center justify-center px-6 pt-20 pb-28">
        <div className="w-full rounded-lg border border-slate-200 bg-white p-6 text-center shadow-sm">
          ...
          <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-600">
            Move your cursor to show a ghost pointer. Highlight, annotate,
            scroll, click, and type actions are sent as permissioned requests.
          </p>
          <div className="mt-8 grid gap-3 text-left md:grid-cols-3">
            <Permission
              label="Highlight"
              enabled={session.permissions.canHighlight}
            />
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-left">
            ...
            <p className="text-sm text-slate-500">
              Recorded timeline stays isolated from live collaboration and is
              available for playback after the session ends.
            </p>
          </div>
          {session.recording ? (
  ...
  return (
    <div className="rounded-md border border-slate-200 p-4">
      <div className="flex items-center justify-between">
        ...
        <span
          className={`h-2.5 w-2.5 rounded-full ${enabled ? 'bg-green-500' : 'bg-slate-300'}`}
        />
      </div>
