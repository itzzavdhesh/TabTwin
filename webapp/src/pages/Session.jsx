...
import { downloadRecording } from '../recording/exportRecording.js';
...
export default function Session({ sessionId }) {
  ...
  const playbackDuration = useMemo(
    () => playbackEngineRef.current?.getDuration() ?? 0,
    [session.recording, playbackCurrentTime],
  );
  ...
  if (
    (session.status === 'error' || session.status === 'offline') &&
    !session.recording &&
    !recordingEnabled
  ) {
  ...
  return (
    <main
      ...
      onPointerMove={cursor.handlePointerMove}
    >
      <div className="absolute inset-x-0 top-0 border-b border-slate-200 bg-white px-5 py-3">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <RecordingBadge
              enabled={recordingEnabled || session.hostRecordingActive}
              state={
                playbackState === 'playing'
                  ? 'playback'
                  : recordingEnabled || session.hostRecordingActive
                    ? 'recording'
                    : 'idle'
              }
            />
      ...
      <section className="mx-auto flex min-h-screen max-w-6xl items-center justify-center px-6 pt-20 pb-28">
        <div className="w-full rounded-lg border border-slate-200 bg-white p-6 text-center shadow-sm">
          <p className="text-sm font-semibold text-teal-700">
            Your actions appear in the host browser
          </p>
          <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-600">
            Move your cursor to show a ghost pointer. Highlight, annotate, scroll, click, and type
            actions are sent as permissioned requests.
          </p>
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-left">
            ...
            <p className="text-sm text-slate-500">
              {session.hostRecordingActive ? 'The host has started recording this session. ' : ''}
              Recorded timeline stays isolated from live collaboration and is available for playback
            ...
            <div className="mt-6 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
              <div className="space-y-3">
                <PlaybackControls
                  ...
                  playback={playbackEngineRef.current}
                  recording={session.recording}
                />
                <button
                  className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-100"
                  onClick={() => downloadRecording(session.recording)}
                  type="button"
                >
                  Download recording (.tabtwin.json)
                </button>
              </div>
      ...
      <GhostCursor
        ...
        y={cursor.position.y}
        name={guestName}
        color={session.guest?.color || '#2563eb'}
      />
      <AnnotationLayer session={session} />
      {playbackState === 'playing' || playbackState === 'paused' ? (
        <PlaybackView annotations={playbackAnnotations} cursors={playbackCursors} />
      ) : null}
  ...
  return (
    <div className="rounded-md border border-slate-200 p-4">
      ...
      <p className="mt-2 text-xs text-slate-500">
        {enabled ? 'Allowed by host' : 'Needs host approval'}
      </p>
    </div>
  );
}
