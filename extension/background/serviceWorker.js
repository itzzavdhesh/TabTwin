...
async function handleMessage(message, sender) {
  switch (message.type) {
    ...
    case 'control:revoke':
      ...
      return snapshot();
    case 'guest:promote':
      sendSocket('guest:promote', { guestId: message.payload.guestId });
      return snapshot();
    case 'agent:run':
...
async function startSession() {
  ...
  state.session = {
    ...
    link: session.link,
    viewerLink: session.viewer_link,
...
async function handleServerEvent({ event, payload = {} }) {
  ...
  if (event === 'guest:promoted') {
    ...
    addLog(`${payload.guest?.name || 'Viewer'} promoted to co-host`);
    return;
  }
