...
export default function Popup() {
  ...
  async function startSession() {
    ...
    try {
      ...
      if (response?.session) {
        ...
        setError(
          response?.activityLog?.[0]?.message ?? 'Failed to start session. Is the server running?',
        );
      }
    } catch (err) {
  ...
  return (
    <main className="min-h-[540px] bg-slate-50 p-4 flex flex-col justify-between">
      <div>
        <header className="flex items-center justify-between border-b border-slate-200 pb-3">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <h1 className="text-xl font-black tracking-tight text-slate-950">TabTwin</h1>
              <p className="text-xs text-slate-500 font-medium">Ghost cursor tab collaboration</p>
            </div>
          </div>
          <button
            className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-100 shadow-sm"
        ...
        {!state.session ? (
          <section className="mt-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-xs leading-relaxed text-slate-600">
              Start a session, share the link, and watch your guest appear as a ghost cursor in the
              active tab.
            </p>
            <button
              className="mt-5 w-full rounded-lg bg-slate-950 px-4 py-3 text-sm font-bold text-white shadow transition hover:bg-slate-800 disabled:bg-slate-400"
              disabled={busy}
      ...
      <footer className="mt-6 border-t border-slate-200 pt-3 text-[11px] text-slate-500 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          />
          <span>
            By <strong>Avdhesh Kumar Dadhich</strong>
          </span>
        </div>
...
function Settings({ state, onBack, onSave }) {
  ...
  return (
    <main className="min-h-[540px] bg-slate-50 p-4">
      <button className="text-sm font-semibold text-slate-600" onClick={onBack} type="button">
        Back
      </button>
      ...
      <button
        ...
        onClick={() => chrome.runtime.sendMessage({ type: 'settings:clear-session' })}
        type="button"
      >
        Clear Session Data
      </button>
