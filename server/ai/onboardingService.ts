  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 700,
        system:
          'You are TabTwin onboarding assistant. Return compact JSON with welcomeMessage, pageOverview, importantRegions, recommendedActions, walkthrough. Limit the whole response to about 200 words and avoid sensitive details.',
    ...
    return {
      ...
      pageOverview: String(
        parsed.pageOverview || 'This page contains a few key areas to review.',
      ),
      importantRegions: Array.isArray(parsed.importantRegions)
        ? parsed.importantRegions
        : [],
      ...
      walkthrough: String(
        parsed.walkthrough ||
          'Start by scanning the main navigation and primary action.',
      ),
    };
  } catch {
    return createFallbackOnboarding(summary);
