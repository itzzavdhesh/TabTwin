...
export function createSignalingHandler({ sessions, redisClient, redisSub, serverId }) {
  ...
  function handleConnection(socket) {
    ...
    // Heartbeat liveness flag, flipped back to true on every pong. Read and
    // reset by attachHeartbeat()'s sweep interval so an abruptly-dropped
    // connection (crashed tab, network cut, no close frame ever sent) is
    // detected and cleaned up instead of lingering until a TCP timeout.
    socket.isAlive = true;
    socket.on('pong', () => {
      socket.isAlive = true;
    });
...
/**
 * Sweeps a WebSocketServer's clients on a fixed interval, ping-ing anyone
 * who answered the previous ping and terminating anyone who didn't.
 * `terminate()` fires the socket's 'close' event, which reuses the existing
 * removeSocket cleanup path — so a dead connection (crashed tab, network
 * drop, no close frame ever sent) is detected and its participant removed
 * within roughly one interval, instead of lingering until a TCP timeout.
 *
 * @param {import('ws').WebSocketServer} wss
 * @param {{ intervalMs?: number }} [options]
 * @returns {NodeJS.Timeout} the interval handle, for tests/manual teardown
 */
export function attachHeartbeat(wss, { intervalMs = HEARTBEAT_INTERVAL_MS } = {}) {
